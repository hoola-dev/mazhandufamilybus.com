...
use ReallySimpleJWT\Token;//for jwt
require 'vendor/autoload.php';//for jwt
if (!isset($_GET['controller']) || empty($_GET['controller']))
{
	header("HTTP/1.1 301 Moved Permanently");
	pjUtil::redirect(PJ_INSTALL_URL . basename($_SERVER['PHP_SELF'])."/busticketbooking");
 
	//pjUtil::redirect(PJ_INSTALL_URL . basename($_SERVER['PHP_SELF'])."?controller=pjHome&action=pjActionHome");
}
