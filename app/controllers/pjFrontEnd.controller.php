...
class pjFrontEnd extends pjFront
{
	...
	public function pjActionSaveBooking() {
		...
		if ($is_api || $this->isXHR ()) {
            ...
            if ($is_api) {
                ...
                $return_status = ($params['is_same_site_return'] == 1) ? 'T' : 'F';
            } else {
                $STORE = @$_SESSION [$this->defaultStore];
			...
			$ticket_price_arr = $pjPriceModel->getTicketPrice($bus_id, $pickup_id, $return_id, $booked_data, $this->option_arr, $this->getLocaleId(), $return_status);
			
			$data ['sub_total'] = $ticket_price_arr['sub_total'];
            $data ['tax'] = $ticket_price_arr['tax'];
            $actual_total = $ticket_price_arr['total'];
            if ($is_api) {   
                $api_discount = $this->option_arr['o_api_discount'];             
                if ($actual_total <= $api_discount) {
                    $api_given_discount = $actual_total;
                    $actual_total = 0;                    
                } else {
                    $actual_total = $actual_total - $api_discount;
                    $api_given_discount = $this->option_arr['o_api_discount'];
                }
                $data ['api_discount'] = $api_given_discount;
            }
			$data ['total'] = $data['sub_total'] = $data['deposit'] = $actual_total;
			...
			if ($id !== false && ( int ) $id > 0) {
				...
				pjFrontEnd::pjActionConfirmSend ( $this->option_arr, $arr, PJ_SALT, 'pending',true,false,$is_api);
				...
				if ($is_return == 'T') {
					...
					pjFrontEnd::pjActionConfirmSend ( $this->option_arr, $return_arr, PJ_SALT, 'pending',true,true,$is_api);
				}
    ...
    public function pjActionConfirmFlutterwave()
	{
		...
		if ($_GET['status'] == 'successful') {
            ...
            if ($response_decode['status'] == 'success' && $booking_arr['deposit'] == $response_decode['data']['amount'] && $currency == $response_decode['data']['currency']) {
        ...
        if ($is_booking_confirmed) {
			...
			if ($booking_arr['is_return'] == 'T') {
                ...
                pjFrontEnd::pjActionConfirmSend ( $this->option_arr, $return_arr, PJ_SALT, 'confirm',true,true);
            }
		} else {
            $pjBookingModel->reset()->setAttributes(array('id' => $booking_arr['id']))->modify(array(
				'status' => 'cancelled',
	...
	public function pjActionCancel()
	{
		...
		if (isset($_POST['booking_cancel']))
		{
			...
			if (count($booking_arr) > 0)
			{
                $is_bus_started = ($booking_arr['booking_datetime'] > date('Y-m-d H:i:s')) ? false : true;
                ...
                $result = $this->requestAction(array('controller' => 'pjCombinedApi', 'action' => 'pjActionCancelInsurance', 'params' => array('uuid'=>$booking_arr['uuid'])), array('return'));
                ...
                if ($user_id) {
                    ...
                    if ($user_arr && $user_arr['role_id'] == 2) {
                        ...
                        $new_credit = $user_current_credit + $booking_arr['total'];
				...
				pjUtil::redirect($_SERVER['PHP_SELF'] . '?controller=pjFrontEnd&action=pjActionCancel&err=200'.$refund_message);
			}
		}else{
    ...
    public function pjActionConfirmBooking($params) {	
		...
		if ($uuid) {
			...
			if (isset($booking_arr1[0]) && count($booking_arr1[0]) > 0) {
				...
				$sql = "UPDATE `".$pjBookingModel->getTable()."` SET status = 'confirmed' WHERE `id` = ".$booking_arr['id'];
				
				$pjBookingModel->reset()->execute($sql);			                
                 
                $this->pjActionConfirmSend($options_arr, $booking_arr, PJ_SALT, 'confirm');
    ...
    public function pjActionConfirmLipilaWallet()
	{
        ...
        $result = file_put_contents('message.txt', "\n\n\n-------Lipila---------\n\n\n".$content."\n\n\n----------------".PHP_EOL , FILE_APPEND | LOCK_EX);

        $response_explode = explode('_',$response_decode['RESPONSE']['EXTID']);
        $uuid = $response_explode[0];
		...
		if (isset($booking_arr1[0]) && count($booking_arr1[0])) {
            $booking_arr = $booking_arr1[0];
