        ...
        if ($pickup_id && $return_id && $booking_date && $bus_id && $seats && $first_name && $last_name && $phone) {
            ...
            if ($ticket_id) {
                ...
                $booked_data = array(
                    ...
                    'return_bus_id'=> $return_bus_id,//for ret
                    'return_selected_seats' => $r_selected_seats,//for ret
                    'return_ticket_cnt_'.$r_ticket_id => $return_seats_count,//for ret
                    'return_selected_ticket' => $return_seats_count,//for ret
                ...
                $store = array(
                    ...
                    'is_return' => $is_return,
                    'return_bus_id_arr' => array($return_bus_id),//for ret
                ...
                //for return
                if(isset($is_return) && ($is_return == 'T')){
                    $return_date = pjUtil::formatDate($return_date, $this->option_arr['o_date_format']);
                        $r_booking_period = array();            

                        $pjBusModel = pjBusModel::factory();
                        $r_bus_id_arr = $pjBusModel->getBusIds($return_date, $return_id, $pickup_id);
        ...
        if($token_validation_result){
            ...
            if(count($booking_ids_arr)){
                foreach($booking_ids_arr as $k => $id){
                    if($id > 0){
                        pjApiBooking::updateStatus($agent_user_id, $id, $status);
                    }
                }
