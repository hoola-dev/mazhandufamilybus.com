...
class pjDummyBook extends pjFront
{
		...
		if ($is_api || $this->isXHR ()) {
			...
			if ($id !== false && ( int ) $id > 0) {
				...
				if ($is_return == 'T') {
					...
					$return_date = ($is_api) ? $params['api_store']['return_date'] :  $this->_get ( 'return_date' );//
					$data ['booking_date'] = pjUtil::formatDate ( $return_date, $this->option_arr ['o_date_format'] );
				...
				$pjBookingTicketModel = pjBookingTicketTestModel::factory ();
				...
				if(isset($params['api_store']['request_id']) && !empty($params['api_store']['request_id'])){
	                ...
	                $payment_id = pjPaymentTestModel::factory($paymentData)->insert()->getInsertId();
	            }
				...
				$tickets = pjBookingTicketTestModel::factory ()->join ( 'pjMultiLang', "t2.model='pjTicket' AND t2.foreign_id=t1.ticket_id AND t2.field='title' AND t2.locale='" . $this->getLocaleId () . "'", 'left outer' )->join ( 'pjTicket', "t3.id=t1.ticket_id", 'left' )->select ( 't1.*, t2.content as title' )->where ( 'booking_id', $arr ['id'] )->findAll ()->getData ();
				...
				pjBookingPaymentTestModel::factory ()->setAttributes ( $payment_data )->insert ();
				...
				if ($is_return == 'T') {
					...
					$return_tickets = pjBookingTicketTestModel::factory ()->join ( 'pjMultiLang', "t2.model='pjTicket' AND t2.foreign_id=t1.ticket_id AND t2.field='title' AND t2.locale='" . $this->getLocaleId () . "'", 'left outer' )->join ( 'pjTicket', "t3.id=t1.ticket_id", 'left' )->select ( 't1.*, t2.content as title' )->where ( 'booking_id', $arr ['back_id'] )->findAll ()->getData ();
	...
	public function pjActionConfirmAuthorize()
	{
		...
		$booking_arr['tickets'] = pjBookingTicketTestModel::factory()
		...
		if (count($booking_arr) > 0)
		{
			...
			if ($response !== FALSE && $response['status'] === 'OK')
			{
				...
				pjBookingPaymentTestModel::factory()
					->where('booking_id', $booking_arr['id'])
				...
				if ($booking_arr['is_return'] == 'T') {
				    ...
				    $return_tickets = pjBookingTicketTestModel::factory ()
				    ->join ( 'pjMultiLang', "t2.model='pjTicket' AND t2.foreign_id=t1.ticket_id AND t2.field='title' AND t2.locale='" . $this->getLocaleId () . "'", 'left outer' )
				    ->join ( 'pjTicket', "t3.id=t1.ticket_id", 'left' )
				    ->select ( 't1.*, t2.content as title' )
	...
	public function pjActionConfirmPaypal()
	{
		...
		if (!empty($booking_arr['back_id'])) {
			$back_arr = pjDummyBookingModel::factory()
				->select('t1.*')
				->find($booking_arr['back_id'])->getData();
	...
	public function pjActionCancel()
	{
		...
		{
			$booking_arr = pjDummyBookingModel::factory()
				->select('t1.*, t2.departure_time, t2.arrival_time, t3.content as route_title, t4.content as from_location, t5.content as to_location')
				->join('pjBus', "t2.id=t1.bus_id", 'left outer')
				->join('pjMultiLang', "t3.model='pjRoute' AND t3.foreign_id=t2.route_id AND t3.field='title' AND t3.locale='".$this->getLocaleId()."'", 'left outer')
				->join('pjMultiLang', "t4.model='pjCity' AND t4.foreign_id=t1.pickup_id AND t4.field='name' AND t4.locale='".$this->getLocaleId()."'", 'left outer')
			...
			if (count($booking_arr) > 0)
			{
				...
				pjUtil::redirect($_SERVER['PHP_SELF'] . '?controller=pjFrontEndMobile&action=pjActionCancel&err=200');
			}
		}else{
				...
				if (count($arr) == 0)
				{
					...
					if ($arr['status'] == 'cancelled')
					{
						...
						if ($_GET['hash'] != $hash)
						{
							...
							{
								$arr['tickets'] = pjBookingTicketTestModel::factory()
									->join('pjMultiLang', "t2.model='pjTicket' AND t2.foreign_id=t1.ticket_id AND t2.field='title' AND t2.locale='".$this->getLocaleId()."'", 'left outer')
									->join('pjTicket', "t3.id=t1.ticket_id", 'left')
									->select('t1.*, t2.content as title')
	...
	public function pjActionPrintTickets()
	{
		...
		if(!empty($arr))
		{
			...
			if($hash == $_GET['hash'])
			{
				...
				{
					$arr['tickets'] = pjBookingTicketTestModel::factory()->join('pjMultiLang', "t2.model='pjTicket' AND t2.foreign_id=t1.ticket_id AND t2.field='title' AND t2.locale='".$this->getLocaleId()."'", 'left outer')
						->join('pjTicket', "t3.id=t1.ticket_id", 'left')
	...
	public function pjActionConfirmSend($option_arr, $booking_arr, $salt, $opt, $is_send_to_admin=true,$is_return=false)
	{
		...
		if(!empty($booking_arr['c_phone']) && $opt == 'payment')
		{
			...
			{
				$message = str_replace($tokens['search'], $tokens['replace'], $lang_message[0]['content']);
				if(isset($booking_arr['payment_method']) && ($booking_arr['payment_method'] == 'mmp')){
					$message = "Your ticket is in Pending status until the payment gateway confirms the transaction as successful. When the ticket got confirmed, you will get an email/SMS.";
				}
				$params = array(
						'text' => $message,
						'type' => 'unicode',
		...
		{  
			...
			$custom_message ='';
			if(isset($booking_arr['payment_method']) && ($booking_arr['payment_method'] == 'mmp')){//mobile money pending
					$custom_message = "Your ticket is in Pending status until the payment gateway confirms the transaction as successful. When the ticket got confirmed, you will get an email/SMS.
			...
			if (count($lang_message) === 1)
			{
                $message = str_replace($tokens['search'], $tokens['replace'], $lang_message[0]['content']);
                if(isset($booking_arr['payment_method']) && ($booking_arr['payment_method'] == 'mmp')){
                	$message = str_replace($tokens['search'], $tokens['replace'], $custom_message);
                }
    ...
    function pjSendTicket() {
        ...
        $booking_phone = pjDummyBookingModel::factory()
