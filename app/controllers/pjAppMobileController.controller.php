...
class pjAppMobileController extends pjController
{
	public $models = array();
	...
	public function getData($option_arr, $booking_arr, $salt, $locale_id)
	{
		...
		$cancelURL = PJ_INSTALL_URL . 'index.php?controller=pjFrontEndMobile&action=pjActionCancel&id='.@$booking_arr['id'].'&hash='.sha1(@$booking_arr['id'].@$booking_arr['created'].$salt);
		...
		$replace = array(
            ((isset($booking_arr['c_title']) && !empty($booking_arr['c_title'])) ? $personal_titles[$booking_arr['c_title']] : null), 
            $booking_arr['c_fname'], 
            $booking_arr['c_lname'], 
            $booking_arr['c_email'], 
            $booking_arr['c_phone'], 
	...
	{
		$user_arr = pjUserModel::factory()->whereIn('t1.role_id', array(1,3))->findAll()->getData();
		$arr = array();
		foreach($user_arr as $v)
		{
			if(!empty($v['email']))
	...
	public function getBusList($pickup_id, $return_id, $bus_id_arr, $booking_period, $booked_data, $date, $is_return) {
		...
		$bus_arr = pjBusModel::factory ()->join ( 'pjMultiLang', "t2.model='pjRoute' AND t2.foreign_id=t1.route_id AND t2.field='title' AND t2.locale='" . $this->getLocaleId () . "'", 'left outer' )->join ( 'pjBusType', "t3.id=t1.bus_type_id", 'left outer' )->select ( " t1.*, t2.content AS route, t3.seats_map,t3.seat_layout,DATE_FORMAT(t1.departure_time,'%r') as f_departure_time, DATE_FORMAT(t1.arrival_time,'%r') as f_arrival_time" )->where("(t1.route_id IN(SELECT `TR`.id FROM `".pjRouteModel::factory()->getTable()."` AS `TR` WHERE `TR`.status='T') )")->where ( "(t1.id IN(" . join ( ',', $bus_id_arr ) . "))" )->index ( "FORCE KEY (`bus_type_id`)" )->orderBy ( "route asc" )->findAll ()->getData ();
