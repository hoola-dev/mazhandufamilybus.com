...
class pjAdminAppUsers extends pjAdmin
{
	...
	public function pjActionCreate()
	{
		...
		if ($this->isAdmin())
		{
			if (isset($_POST['user_create']))
			{
				...
				pjUtil::redirect($_SERVER['PHP_SELF'] . "?controller=pjAdminAppUsers&action=pjActionIndex&err=$err");
			} else {
				...
				$this->appendJs('pjAdminAppUsers.js');
			}
		} else {
			$this->set('status', 2);
		}
        ...
        if ($this->isAdmin())
		{
            if ($this->isXHR())
            {
                ...
                if ($_GET['id'] != $this->getUserId() && $_GET['id'] != 1)
                {
                    ...
                    {
                        $pjUserExtraInfoModel = pjUserExtraInfoModel::factory();
                        $pjUserExtraInfoModel->where('t1.user_id =',$_GET['id']);
                    
                        $arr1 = $pjUserExtraInfoModel->findAll()->getData();
                        $arr = (count($arr1)) ? $arr1[0] : array();
                        if(isset($arr['id']) && ($arr['id'] > 0)){
                            pjUserExtraInfoModel::factory()->setAttributes(array('id' => $arr['id']))->erase();
                        }
	...
	public function pjActionGetUser()
	{
        ...
        if ($this->isAdmin())
		{
            if ($this->isXHR())
            {
                ...
                $data_object->where('(role_id = 6)');// fetch app users with role id 6(Normal users)
	...
	public function pjActionIndex()
	{
		...
		if ($this->isAdmin())
		{
			...
			$this->appendJs('pjAdminAppUsers.js?'.PJ_CSS_JS_VERSION);
	...
	public function pjActionUpdate()
	{
		...
		if ($this->isAdmin())
		{
			...
			if (isset($_POST['user_update']))
			{
                ...
                if ($this->isBusAdmin()) {
                    $pjUserModel->where('(t1.id = '.$this->getUserId().' OR t1.id IN (SELECT id FROM '.$pjUserModel->getTable().' WHERE added_by = '.$this->getUserId().') OR t1.id IN (SELECT id FROM '.$pjUserModel->getTable().' WHERE added_by IN (SELECT id FROM '.$pjUserModel->getTable().' WHERE added_by = '.$this->getUserId().')))');
                }
				...
				if (count($arr) === 0)
				{
					pjUtil::redirect(PJ_INSTALL_URL. "index.php?controller=pjAdminAppUsers&action=pjActionIndex&err=AU08");
                }
                ...
                //to update data in users_extra_info table
                $dataExtra = array(
                    'title' => $_POST['title'],
                    'first_name' => $_POST['first_name'],
                    'last_name' => $_POST['last_name']
                );
                $result_ext_modify = pjUserExtraInfoModel::factory()->where('id', $_POST['idExtra'])->limit(1)->modifyAll($dataExtra);
				...
				pjUtil::redirect(PJ_INSTALL_URL . "index.php?controller=pjAdminAppUsers&action=pjActionIndex&err=AU01");
                ...
                $pjUserExtraInfoModel->where('t1.user_id =',$_GET['id'])
                ->select ('t1.*');
                $arr1Extra = $pjUserExtraInfoModel->findAll()->getData();
                $arrExtra = (count($arr1Extra)) ? $arr1Extra[0] : array();


				$this->set('arr', $arr);
                $this->set('arrExtra', $arrExtra);
        ...
        if ($this->isAdmin()) {
            ...
            if (count($arr) === 0)
            {
                ...
                $user = pjUserModel::factory()
                    ...
                    if (count($user) != 1) {
                        pjUtil::redirect($_SERVER['PHP_SELF'] . "?controller=pjAdminAppUsers&action=pjActionCredit&id=".$user_id."&&err=ACU01");
                    }
            }
            ...
            if ($user_id) {
                ...
                if ($result) {
                    ...
                    pjUtil::redirect(PJ_INSTALL_URL. "index.php?controller=pjAdminAppUsers&action=pjActionIndex");
                } else {
                    pjUtil::redirect(PJ_INSTALL_URL. "index.php?controller=pjAdminAppUsers&action=pjActionCredit&id=".$user_id."&&err=ACU02");
                }
            } else {
                pjUtil::redirect(PJ_INSTALL_URL. "index.php?controller=pjAdminAppUsers&action=pjActionIndex");
            }
