...
class pjApiAgents extends pjFront
{
        ...
        if($token_validation_result){
            ...
            if(count($agentsBookings)){
                foreach($agentsBookings as $key => $booking){
                    ...
                    $arr['from_name'] = trim($locNameArr[0]);
                    $arr['to_name'] = trim($locNameArr[1]);
                    ...
                    $booking_date_val = $booking_datetime_arr[0];
                    $booking_date_arr = explode("-", $booking_date_val);
                    $booking_date_formatted = $booking_date_arr[2].'-'.$booking_date_arr[1].'-'.$booking_date_arr[0];

                    $arr['booking_date'] = $booking_date_formatted;
    ...
    public function pjGetCreditData(){
        ...
        if($token_validation_result){
            ...
            $data['local_bus_discount'] = number_format($this->option_arr['o_local_bus_discount'], 2);
    ...
    //api for titles
    public function pjGetTitles(){
        $data = array(
            array('id' => '0','name' => 'Choose'),
            array('id' => '1','name' => 'Mr.'),
            array('id' => '2','name' => 'Mrs.'),
            array('id' => '3','name' => 'Ms.'),
            array('id' => '4','name' => 'Dr.'),
    ...
    public function pjActionMyBookingsV2(){
        $agent_user_id = (isset($_POST['agent_user_id']) && trim($_POST['agent_user_id'])) ? trim($_POST['agent_user_id']) : '';
        $token = (isset($_POST['token']) && trim($_POST['token'])) ? trim($_POST['token']) : '';
        $status = (isset($_POST['status']) && trim($_POST['status'])) ? trim($_POST['status']) : '';
        $status = (isset($status) && $status == 'all') ? '' : $status;
        ...
        if($token_validation_result){
            ...
            if(count($agentsBookings)){
                foreach($agentsBookings as $key => $booking){
                    ...
                    $arr['booking_id'] = $agentsBookings[$key]['id'];
                    $arr['unique_id'] = $agentsBookings[$key]['uuid'];
                    $arr['group_identifier'] = ($agentsBookings[$key]['group_identifier']) ?? "";
                    ...
                    $arr['discount'] = $agentsBookings[$key]['discount_on_ticket'];
                    $ticket_price = $agentsBookings[$key]['total']+ $agentsBookings[$key]['discount_on_ticket'];
                    $arr['ticket_price'] = strval(number_format($ticket_price,2));
                    $arr['booking_date'] = $booking_date_formatted;
                    $arr['booking_time'] = $agentsBookings[$key]['booking_time'];
                    array_push($respArr, $arr);
                }
    ...
    public function pjActionOutBookingsV2(){
        ...
        if($token_validation_result){
            ...
            $data = $pjBookingModel
                        ->select('t1.*, t2.departure_time, t2.arrival_time, t3.content as route_title, t4.content as from_location, t5.content as to_location,DATE(t1.booking_datetime) as booked_for_dt')
                        ->orderBy("t1.id DESC")
                        ->findAll()
                        ->getData();
            ...
            foreach($data as $k => $v)
            {
                ...
                $arr['booking_id'] = $v['id'];
                $arr['unique_id'] = $v['uuid'];
                $arr['group_identifier'] = ($v['group_identifier']) ?? "";

                //set print limit if the booking dont have that entry(if booking is for today or upcoing date)
                $today = date('Y-m-d');
                $date1 = new DateTime($today);
                $date2 = new DateTime($v['booked_for_dt']);

                $printRes = pjPrintSettingsModel::factory()
                ->where('booking_id', $v['id'])
                ->findAll()
                ->getData();
                if(($date2 >= $date1) && (count($printRes) == 0)){
                    $printData = array(
                        'group_identifier' => $arr['group_identifier'],
                        'booking_id' => $v['id'],
                        'print_limit' => 2
                    );
                ...
                $arr['discount'] = $v['discount_on_ticket'];
                $ticket_price = $v['total'] + $v['discount_on_ticket'];
                $arr['ticket_price'] = strval(number_format($ticket_price,2));
    ...
    public function pjDiscountBookings(){
        $agent_user_id = (isset($_POST['agent_user_id']) && trim($_POST['agent_user_id'])) ? trim($_POST['agent_user_id']) : '';
        $token = (isset($_POST['token']) && trim($_POST['token'])) ? trim($_POST['token']) : '';
        if(empty($agent_user_id) || empty($token)){
            $resp = array(
        ...
        if($token_validation_result){
            //get selected agent's bookings
            $pjBookingModel = pjBookingModel::factory() 
                ->join('pjDiscount', "t2.booking_id=t1.id", 'inner')               
                ->join('pjBus', "t3.id=t1.bus_id", 'left outer')
                ->join('pjMultiLang', "t4.model='pjRoute' AND t4.foreign_id=t3.route_id AND t4.field='title' AND t4.locale='".$this->getLocaleId()."'", 'left outer')
                ->join('pjMultiLang', "t5.model='pjCity' AND t5.foreign_id=t1.pickup_id AND t5.field='name' AND t5.locale='".$this->getLocaleId()."'", 'left outer')
                ->join('pjMultiLang', "t6.model='pjCity' AND t6.foreign_id=t1.return_id AND t6.field='name' AND t6.locale='".$this->getLocaleId()."'", 'left outer')
                ->select("t1.*, t2.discount");

            $pjUserModel = pjUserModel::factory();
            $discountBookings = $pjBookingModel->where('(t2.added_by= '.$agent_user_id.')')
            ->orderBy("t1.id DESC")
            ->findAll()
            //->debug(true)
            ->getData();
            $respArr = array();
            if(count($discountBookings)){
                foreach($discountBookings as $key => $booking){

                    /*echo "<pre>";
                    print_r($booking);
                    exit;*/
                    $arr = array();
                    $arr['booking_id'] = $discountBookings[$key]['id'];
                    $arr['unique_id'] = $discountBookings[$key]['uuid'];
                    $arr['group_identifier'] = ($discountBookings[$key]['group_identifier']) ?? "";

                    //booked seats
                    $pjBookingSeatModel = pjBookingSeatModel::factory();
                    $seat_pair_arr = $pjBookingSeatModel->reset()->where('booking_id', $discountBookings[$key]['id'])->findAll()->getDataPair("seat_id", 'seat_id');
                    $selected_seats = array();
                    $arr['booked_seat_numbers'] = '';
                    if(!empty($seat_pair_arr))
                    {
                    ...
                    $arr['c_title'] = $discountBookings[$key]['c_title'];
                    $arr['c_fname'] = $discountBookings[$key]['c_fname'];
                    $arr['c_lname'] = $discountBookings[$key]['c_lname'];
                    $arr['c_phone'] = $discountBookings[$key]['c_phone'];
                    $arr['c_email'] = $discountBookings[$key]['c_email'];

                    $booking_route = strip_tags($discountBookings[$key]['booking_route']);
                    $bus_name_arr = explode(",",$booking_route);
                    $arr['bus_name'] = $bus_name_arr[0];
                    ...
                    $arr['status'] = $discountBookings[$key]['status'];
                    $arr['total_amount'] = $discountBookings[$key]['total'];
                    $booking_datetime_arr = explode(" ", $discountBookings[$key]['booking_datetime']);
                    $booking_date_val = $booking_datetime_arr[0];
                    $booking_date_arr = explode("-", $booking_date_val);
                    $booking_date_formatted = $booking_date_arr[2].'-'.$booking_date_arr[1].'-'.$booking_date_arr[0];

                    $arr['booking_date'] = $booking_date_formatted;
                    $arr['booking_time'] = $discountBookings[$key]['booking_time'];
                    $arr['discount'] = $discountBookings[$key]['discount'];
                    array_push($respArr, $arr);
                }
            }   
            $resp = array(
