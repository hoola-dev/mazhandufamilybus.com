...
class pjApiBuses extends pjFrontMobile
{
    ...
    */
    public function listBuses() {
        ...
        $data = $pjBusModel->select("t1.id, t1.route_id,t1.start_date, t1.end_date, t1.departure_time,
           ...
           foreach($data as $key => $bus)
            {
                ...
                if($busDisplay == 'true'){    
                    ...
                    //if($arrivalDateTimeStamp >= $todayDateTimeStamp) //commented
                    if($secDepartTimeValue >= $todayDateTimeStamp){
                        if(isset($bus['delayed_by']) && ($bus['delayed_by'] != '')){
                            ...
                            if($updatedDtArr[0] == $today){//show only if there is delay set for current date
                                ...
                                if($delayedByArr[0] == '00'){
                                    $bus_arr['delayed_by'] = $delayedByArr[0].' Hour(s), '.$delayedByArr[1].' Minutes';
                                }else{
                                   $bus_arr['delayed_by'] = ltrim($delayedByArr[0], "0").' Hour(s), '.$delayedByArr[1].' Minutes'; 
                                }
    ...
    */
    public function setDelay() {
        ...
        if($token_validation_result){
            ...
            ->select('id,delayed_by,DATE(updated_dt) as updated_dt')
            ->where('bus_id', $bus_id)
            ->limit(1)
            ->findAll()
            ->getData();
            if((count($busDelayRes)) && isset($busDelayRes[0]) && !empty($busDelayRes[0])){
                ...
                $oldDelay = 0;
                if($busDelay['updated_dt'] == date('Y-m-d')){
                   ...
                   $finalTime = $finalTime.":00";
                    ...
                    $finalTime = $delay_time_value;
                }

                $maxTime ="3:00:00";//max 3 hours delay only can be set
                if (strtotime($finalTime) <= strtotime($maxTime)) {
                    //update delay
                    $pjBusDelayModel = pjBusDelayModel::factory();       
                    ...
                    $response = array(
                        'status' => 'false',
                        'message' => 'Delay exceeded the maximum allowed limit of 3 hours'
                    );
                    pjAppController::jsonResponse($response);
                }
        ...
        if ($this->isBusReady() == true)
        {
            ...
            if($bus_id_arr)
            {
                ...
                foreach($bus_arr as $bus)
                {
                    ...
                    $arrDept = $pjBusLocationModel->reset()->select("DATE_FORMAT(departure_time,'%T') as f_departure_time")->where('bus_id', $bus['id'])->where("location_id", $pickup_id)->limit(1)->findAll()->getData ();
                    $arr['departure_time'] = $arrDept[0]['f_departure_time'];

                    $arrArrive = $pjBusLocationModel->reset()->select("DATE_FORMAT(arrival_time,'%T') as f_arrival_time")->where('bus_id', $bus['id'])->where("location_id", $return_id)->limit(1)->findAll()->getData ();
                    $arr['arrival_time'] = $arrArrive[0]['f_arrival_time'];
                    ...
                    if(isset($delayData[0]) && isset($delayData[0]['delayed_by']) && ($delayData[0]['delayed_by'] != '' && ($delayData[0]['updated_dt'] == date('Y-m-d')))){
                        ...
                        if($delayedByArr[0] == '00'){
                            $arr['delayed_by'] =  $delayedByArr[0] . " Hour(s), " . $delayedByArr[1] . " Minutes";
                        }else{
                            $hourVal = ltrim($delayedByArr[0], "0");
                            $arr['delayed_by'] =  $hourVal . " Hour(s), " . $delayedByArr[1] . " Minutes";
                        }
                        ...
                        $arr['departure_time'] = $finalDepartTime.':00';
                        $arr['arrival_time'] = $finalArrivalTime.':00';
                    }
    ...
    public function getBusIds($date, $pickup_id, $return_id)
    {
        ...
        $arr = pjBusModel::factory()
            ...
            ->where("(t1.route_id IN(SELECT `TR`.id FROM `".pjRouteModel::factory()->getTable()."` AS `TR` WHERE `TR`.status='T'))")
            //->where("(DATE(t2.updated_dt) = CURDATE())")
            ->select("t1.*,t2.delayed_by,DATE(t2.updated_dt) as updated_date,NOW() as curr_datetime")
            //->debug(true)
            ->findAll()
            ->getData();
        ...
        foreach($arr as $k => $v)
        {
            $pickup_arr = $pjBusLocationModel->reset ()->where ( 'bus_id', $v['id'] )->where ( "location_id", $pickup_id )->limit ( 1 )->findAll ()->getData ();
            if (! empty ( $pickup_arr )) {
                //echo $v['id']."  ".$v['delayed_by']."<br/>";
                
                $departure_dt = $date . ' ' . $pickup_arr [0] ['departure_time'];
                $dt_compare = date('Y-m-d H:i:s');
                $today_dt = date('Y-m-d');
                //echo $v['id'].":".$v['updated_date']." --- ".$today_dt."<br/>";

                //echo $date."-".$v['updated_date'];exit;
                if(isset($v['delayed_by']) && !empty($v['delayed_by']) && ($v['updated_date'] == $today_dt) && ($date == $v['updated_date'])){
                    //echo "tt";
                    //echo $v['id']."test<br/>";
                    ...
                    $departure_dt1 = $today_dt . ' ' . $pickup_arr [0] ['departure_time'];
                    $dt_compare1 = strtotime("+".$delayTimeArr[0]." hours +".$delayTimeArr[1]." minutes", strtotime($departure_dt1));
                    $dt_compare1 = date( "Y-m-d H:i:s", $dt_compare1);

                    
                   // echo $v['id']."**".$dt_compare1."===".$v['curr_datetime']."<br/>";
                    if($v['curr_datetime'] <= $dt_compare1)
                    {
                        //echo "ttt";
                        $bus_id_arr[] = $v['id'];
                    }
    ...
    */
    ...
    /*
      * To check whether available or not when providing pickup_id, return_id and date
    */
    public function pjActionBusAvailabilityV2() {
        if(empty($_GET['pickup_id']) || empty($_GET['return_id']) || empty($_GET['date'])){
            $response = array(
                'status' => 'false',
                'message' => 'Please provide all required parameters'
           );
        ...
        if($_GET['pickup_id'] != $_GET['return_id'])
        {
            ...
            if(isset($_GET['is_return']) && ($_GET['is_return'] == 'T')){
                ...
                $return_id = $_GET['pickup_id'];
                $date = pjUtil::formatDate($_GET['return_date'], $this->option_arr['o_date_format']);
                $return_bus_id_arr = pjApiBuses::getBusIds($date, $pickup_id, $return_id);
