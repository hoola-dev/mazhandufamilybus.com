...
class pjFrontEndMobile extends pjFrontMobile
{
		...
		if ($this->isXHR() || isset($_GET['_escaped_fragment_']))
		{
			...
			if($_GET['pickup_id'] != $_GET['return_id'])
			{
				...
				if(empty($bus_id_arr))
				{
					...
					pjAppMobileController::jsonResponse($resp);
				}
	...
	public function pjActionSaveForm()
	{
		...
		if ($this->isXHR())
		{
			...
			{
				pjAppMobileController::jsonResponse(array('status' => 'ERR', 'code' => 110));
            }
            //echo "<pre>here2";
	...
	public function pjActionSaveBooking() {
		...
		if ($is_api || $this->isXHR ()) {
			...
			$booking_time_value = join ( " - ", $bt_arr );
			if(isset($params['user_id']) && ($params['user_id'] > 0) && ($params['is_user'] == 'F') && (isset($params['include_delay']) && $params['include_delay'] == 'T')){
                ...
                if(isset($delayData[0]) && ($delayData[0]['delayed_by'] != '')){
		            ...
		            $booking_time_value = $finalDepartTime." - ".$finalArrivalTime;

		            $timeValueArr = explode(' ', $data ['booking_datetime']);
		            $updatedTimeValue = strtotime("+".$delayedByArr[0]." hours +".$delayedByArr[1]." minutes", strtotime($timeValueArr[1]));
		            $updatedTimeValue = date( "H:i:s", $updatedTimeValue);
		            $data['booking_datetime'] = $timeValueArr[0]." ".$updatedTimeValue;

		            $stopDtTimeValueArr = explode(' ', $data ['stop_datetime']);
		            $updatedStopDtTimeValue = strtotime("+".$delayedByArr[0]." hours +".$delayedByArr[1]." minutes", strtotime($stopDtTimeValueArr[1]));
		            $updatedStopDtTimeValue = date( "H:i:s", $updatedStopDtTimeValue);
		            $data['stop_datetime'] = $stopDtTimeValueArr[0]." ".$updatedStopDtTimeValue;
                }
			}
			$data ['booking_time'] = $booking_time_value;
			...
			if ($id !== false && ( int ) $id > 0) {
				...
				if ($is_return == 'T') {
					...
					if(isset($params['user_id']) && ($params['user_id'] > 0) && ($params['is_user'] == 'F') && (isset($params['include_delay']) && $params['include_delay'] == 'T')){
						//get delay time
						$delayData = pjBusDelayModel::factory()
		                    ->where("(DATE(updated_dt) = CURDATE() and bus_id = '" . $child_bus_arr ['id'] . "')")
		                    ->select("delayed_by")->findAll()->getData(); 
		                if(isset($delayData[0]) && ($delayData[0]['delayed_by'] != '')){
		                	$busDelayInfo = $delayData[0];
				            $delayedByArr = explode(':', $busDelayInfo['delayed_by']);
					...
					$payment_booking_ids .= ','.$back_insert_id;
				...
				//add to payments table for mobile money payment	
				if(isset($params['api_store']['request_id']) && !empty($params['api_store']['request_id'])){
					//insert to payments table
	                $paymentData = array(
	                    'transction_id' => $params['api_store']['request_id'],
	                    'booking_ids' => $payment_booking_ids,
	                    'status' => 'F',
	                    'created_dt' => date("Y-m-d H:i:s")
	                );
	                $payment_id = pjPaymentModel::factory($paymentData)->insert()->getInsertId();
	            }
				...
				pjFrontEndMobile::pjActionConfirmSend ( $this->option_arr, $arr, PJ_SALT, 'pending',true,false,$is_api);
				...
				if ($is_return == 'T') {
					...
					pjFrontEndMobile::pjActionConfirmSend ( $this->option_arr, $return_arr, PJ_SALT, 'pending',true,true,$is_api);
				}
				...
				$additional_data['payment_mode'] = $arr['payment_method'];
            ...
            if ($is_api) {
                ...
                pjAppMobileController::jsonResponse ( $json );
            }
		}
	}
	...
	public function pjActionConfirmAuthorize()
	{
		...
		if (count($booking_arr) > 0)
		{
			...
			if ($response !== FALSE && $response['status'] === 'OK')
			{
				...
				pjFrontEndMobile::pjActionConfirmSend($this->option_arr, $booking_arr, PJ_SALT, 'payment');
				if ($booking_arr['is_return'] == 'T') {
				    ...
				    pjFrontEndMobile::pjActionConfirmSend ( $this->option_arr, $return_arr, PJ_SALT, 'payment' );
				}
			} elseif (!$response) {
	...
	public function pjActionConfirmPaypal()
	{
		...
		if ($response !== FALSE && $response['status'] === 'OK')
		{
			...
			if ($booking_arr['is_return'] == 'T') {
			    ...
			    pjFrontEndMobile::pjActionConfirmSend ( $this->option_arr, $return_arr, PJ_SALT, 'payment',true,true );
			}
		} elseif (!$response) {
			$this->log('Authorization failed');
		} else {
    ...
    public function pjActionConfirmFlutterwave()
	{
		...
		if ($_GET['status'] == 'successful') {
            ...
            CURLOPT_HTTPHEADER => array(
                ...
                "Authorization: Bearer ".$this->option_arr['o_flutterwave_secret_key']
            ),
            ));
            ...
            if ($response_decode['status'] == 'success' && $booking_arr['deposit'] == $response_decode['data']['amount'] && $this->option_arr['o_currency'] == $response_decode['data']['currency']) {
        ...
        if ($is_booking_confirmed) {
            ...
            pjFrontEndMobile::pjActionConfirmSend($this->option_arr, $booking_arr, PJ_SALT, 'confirm');
			...
			if ($booking_arr['is_return'] == 'T') {
                ...
                pjFrontEndMobile::pjActionConfirmSend ( $this->option_arr, $return_arr, PJ_SALT, 'confirm',true,true);
            }
		} else {
            $pjBookingModel->reset()->setAttributes(array('id' => $booking_arr['id']))->modify(array(
				'status' => 'cancelled',
    ...
    private function pjActionFlutterwaveRefund($txn_id,$amount) {
        $curl = curl_init();

        curl_setopt_array($curl, array(
            CURLOPT_URL => "https://api.flutterwave.com/v3/transactions/$txn_id/refund",
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => "",
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 0,
	...
	public function pjActionCancel()
	{
		...
		if (isset($_POST['booking_cancel']))
		{
			...
			if (count($booking_arr) > 0)
			{
                ...
                $refund_message = '';
                if ($booking_arr['payment_method'] == 'flutterwave') {
                    $refund_status = $this->pjActionFlutterwaveRefund($booking_arr['txn_id'],$booking_arr['total']);
                    $refund_message = ($refund_status) ? '&p=1&r=1' : '&p=1&r=0';
                }   
				
				pjFrontEndMobile::pjActionConfirmSend($this->option_arr, $booking_arr, PJ_SALT, 'cancel');
				
				pjUtil::redirect($_SERVER['PHP_SELF'] . '?controller=pjFrontEndMobile&action=pjActionCancel&err=200'.$refund_message);
			}
		}else{
		...
		if(!empty($arr))
		{
			...
			if($hash == $_GET['hash'])
			{
				if($arr['status'] == 'confirmed')
				{
					...
					$data = pjAppMobileController::getTemplate($this->option_arr, $arr, PJ_SALT, $this->getLocaleId());
                    
                    $qr_code_data = PJ_INSTALL_URL.'index.php?controller=pjFrontEndMobile&action=pjActionViewTicket&id='.$print_ticket_id.'&hash='.$print_ticket_hash;            
	...
	public function pjActionConfirmSend($option_arr, $booking_arr, $salt, $opt, $is_send_to_admin=true,$is_return=false,$is_api=false)
	{
		$Email = new pjEmail();
		...
		$tokens = pjAppMobileController::getData($option_arr, $booking_arr, PJ_SALT, $this->getLocaleId());
        ...
        /*wallet message for website starts*/  
        if ($booking_arr['payment_method'] == 'wallet' && $opt == 'pending') {
            $sms_field = 'o_sms_wallet_site_notify_message';
            $lang_message = $pjMultiLangModel->reset()->select('t1.*')
                    ->where('t1.model','pjOption')
                    ->where('t1.locale', $locale_id)
                    ->where('t1.field', $sms_field)
            ...
            if ($is_return) {
                $_SESSION['wallet_message'] = $_SESSION['wallet_message'].'<div style="width:100%;margin-bottom:30px;border-bottom:1px solid #999999">&nbsp;</div>'.$message;
            } else {
                $_SESSION['wallet_message'] = $message;
            }
        }
        /*wallet message for website ends*/  
		...
		{  
            $is_message = true;
            if ($is_api && $booking_arr['payment_method'] != 'cash') {
                $is_message = false;
            }
            if ($is_message) {
                ...
                $email_subject = ($booking_arr['payment_method'] == 'cash') ? 'Your free seat reservation' : $lang_subject[0]['content'];
    ...
    function pjSendTicket() {
        ...
        if (count($booking_phone)) {
            ...
            foreach ($booking_phone as $booking) {
                ...
                if ($status != 'cancel') {
                    $sent = true;
                    pjFrontEndMobile::pjActionConfirmSend($this->option_arr, $booking_arr, PJ_SALT, $status, false);
                }    
            }
            if ($sent) {
                if ($is_api) {
    ...
    public function pjActionCancelBooking($params) {	
		...
		if ($uuid) {
			...
			if (isset($booking_arr1[0]) && count($booking_arr1[0]) > 0) {
                ...
                $this->pjActionConfirmSend($options_arr, $booking_arr, PJ_SALT, 'cancel');
    ...
    public function pjActionConfirmLipilaWallet()
	{
        $this->setAjax(true);
        
        $content = file_get_contents('php://input');
        $response_decode = json_decode($content, true);

        $uuid = $response_decode['RESPONSE']['EXTID'];
        $amount = $response_decode['RESPONSE']['AMOUNT'];
		
		$pjBookingModel = pjBookingModel::factory();
		$booking_arr1 = $pjBookingModel
			->select('t1.*, t2.departure_time, t2.arrival_time, t3.content as route_title, t4.content as from_location, t5.content as to_location')
			->join('pjBus', "t2.id=t1.bus_id", 'left outer')
			->join('pjMultiLang', "t3.model='pjRoute' AND t3.foreign_id=t2.route_id AND t3.field='title' AND t3.locale='".$this->getLocaleId()."'", 'left outer')
			->join('pjMultiLang', "t4.model='pjCity' AND t4.foreign_id=t1.pickup_id AND t4.field='name' AND t4.locale='".$this->getLocaleId()."'", 'left outer')
        ...
        $booking_arr = $booking_arr1[0];
        if ($response_decode['RESPONSE']['RESPONSECODE'] == 100) {	
            $booking_arr['tickets'] = pjBookingTicketModel::factory()
			->join('pjMultiLang', "t2.model='pjTicket' AND t2.foreign_id=t1.ticket_id AND t2.field='title' AND t2.locale='".$this->getLocaleId()."'", 'left outer')
			->join('pjTicket', "t3.id=t1.ticket_id", 'left')
			->select('t1.*, t2.content as title')
			...
			$pjBookingModel->reset()->setAttributes(array('id' => $booking_arr['id']))->modify(array(
				'status' => $this->option_arr['o_payment_status'],
				'txn_id' => $response_decode['RESPONSE']['TXID'],
				'processed_on' => ':NOW()'
			));
			if (!empty($booking_arr['back_id'])) 
			{
            ...
            $tokens = pjAppMobileController::getData($this->option_arr, $booking_arr, PJ_SALT, $this->getLocaleId());
            $pjMultiLangModel = pjMultiLangModel::factory();
            $locale_id = isset($booking_arr['locale_id']) && (int) $booking_arr['locale_id'] > 0 ? (int) $booking_arr['locale_id'] : $this->getLocaleId();
            ...
            $lang_message = $pjMultiLangModel->reset()->select('t1.*')
                            ->where('t1.model','pjOption')
                            ->where('t1.locale', $locale_id)
                            ->where('t1.field', 'o_sms_wallet_failed_message')
                            ->limit(0, 1)
                            ->findAll()->getData();
	...
	public function pjActionSaveBookingV2() {
        $params = $this->getParams();
		...
		if ($is_api || $this->isXHR ()) {
            ...
            if ($is_api) {
                $payment = !empty($params['payment_method']) ? $params['payment_method'] : "";
            } else {
                $payment = 'none';
                if (isset ( $FORM ['payment_method'] )) {
                    if ($FORM ['payment_method'] && $FORM ['payment_method'] == 'creditcard') {
			...
			$data ['total'] = $ticket_price_arr['total'];
            //$data ['deposit'] = $ticket_price_arr['deposit'];
            $data ['deposit'] = $data ['total'];//changed like admin
            $c_phone = ltrim(trim($FORM['c_phone']),0);

            //set discount
            if(isset($params['discount_per_head']) && $params['discount_per_head'] > 0){
            	$max_discount = $this->option_arr['o_local_bus_discount'];
            	if ($params['discount_per_head'] > 0 && $max_discount >= $params['discount_per_head'] && $data ['total'] >= $params['discount_per_head']) {
                    $data ['sub_total'] = $data ['sub_total'] - $params['discount_per_head'];
                    $data['total'] = $data['total'] - $params['discount_per_head'];
                    $data['deposit'] = $data['deposit'] - $params['discount_per_head'];
                    $data['discount_on_ticket'] = $params['discount_per_head'];
                } 
            }
			...
			if(isset($params['group_identifier']) && (!empty($params['group_identifier']))){
					$data ['group_identifier'] = $params['group_identifier'];
				}		
			...
			if(isset($params['discount_per_head']) && $params['discount_per_head'] > 0) {$discount_item = array();
                $discount_item['booking_id'] = $id;
                $discount_item['discount'] = $params['discount_per_head'];               
                $discount_item['added_by'] = $params['user_id'];
                $discount_item['added_on'] = date("Y-m-d H:i:s");
                $disc_result = pjDiscountModel::factory($discount_item)->insert()->getInsertId();
            }
			...
			$print_id = pjPrintSettingsModel::factory($printData)->insert()->getInsertId();
			...
			if ($id !== false && ( int ) $id > 0) {
				...
				if ($is_return == 'T') {
					...
					$data ['total'] = isset($return_ticket_price_arr['total']) ? $return_ticket_price_arr['total'] : 0;
					//$data ['deposit'] = isset($return_ticket_price_arr['deposit']) ? $return_ticket_price_arr['deposit'] : 0;
					$data ['deposit'] = isset($return_ticket_price_arr['total']) ? $return_ticket_price_arr['total'] : 0;//changed like admin
					if(isset($params['user_id']) && ($params['user_id'] > 0) && ($params['is_user'] == 'F')){
						$data ['user_id'] = $params['user_id'];
					}
					...
					if(isset($params['discount_per_head']) && $params['discount_per_head'] > 0) {
					    $discount_item_ret = array();
					    $discount_item_ret['booking_id'] = $back_insert_id;
					    $discount_item_ret['discount'] = $params['discount_per_head'];               
					    $discount_item_ret['added_by'] = $params['user_id'];
					    $discount_item_ret['added_on'] = date("Y-m-d H:i:s");
					    $disc_result = pjDiscountModel::factory($discount_item_ret)->insert()->getInsertId();
					}
					...
					$print_id_ret = pjPrintSettingsModel::factory($printDataRet)->insert()->getInsertId();
				...
				$arr = $pjBookingModel->reset ()->select ( 't1.*, t2.departure_time, t2.arrival_time, t3.content as route_title, t4.content as from_location, t5.content as to_location,DATE_FORMAT(t2.departure_time,\'%T\') as f_departure_time, DATE_FORMAT(t2.arrival_time,\'%T\') as f_arrival_time' )->join ( 'pjBus', "t2.id=t1.bus_id", 'left outer' )->join ( 'pjMultiLang', "t3.model='pjRoute' AND t3.foreign_id=t2.route_id AND t3.field='title' AND t3.locale='" . $this->getLocaleId () . "'", 'left outer' )->join ( 'pjMultiLang', "t4.model='pjCity' AND t4.foreign_id=t1.pickup_id AND t4.field='name' AND t4.locale='" . $this->getLocaleId () . "'", 'left outer' )->join ( 'pjMultiLang', "t5.model='pjCity' AND t5.foreign_id=t1.return_id AND t5.field='name' AND t5.locale='" . $this->getLocaleId () . "'", 'left outer' )->find ( $id )->getData ();
				...
				$additional_data['to_location_name'] = $arr['to_location'];
				$additional_data['total_price'] = $arr['total'];
				$additional_data['discount'] = $arr['discount_on_ticket'];
				$additional_data['travel_date'] = $booking_date;
				$additional_data['total_no_of_seatsbooked'] = count($seat_id_arr);
				$additional_data['departure_time'] = $arr['f_departure_time'];
				$additional_data['arrival_time'] = $arr['f_arrival_time'];
				$additional_data['payment_mode'] = $arr['payment_method'];
				$additional_data['group_identifier'] = $arr['group_identifier'];
				...
				if ($is_return == 'T'){
					...
					$additional_data['r_to_location_name'] = $return_arr['to_location'];
					$additional_data['r_total_price'] = $return_arr['total'];
					$additional_data['r_discount'] = $return_arr['discount_on_ticket'];
					$additional_data['r_departure_time'] = $return_arr['f_departure_time'];
					$additional_data['r_arrival_time'] = $return_arr['f_arrival_time'];
					$additional_data['r_group_identifier'] = $return_arr['group_identifier'];
				}
