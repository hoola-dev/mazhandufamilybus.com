	...
	public function pjActionGetUser()
	{
        ...
        if ($this->isAdmin() || $this->isBusAdmin() || $this->isSuperAgent())
		{
            if ($this->isXHR())
            {
                ...
                if ($this->isBusAdmin()) {
                    $pjUserModel->where('(id = '.$this->getUserId().' OR id IN (SELECT id FROM '.$pjUserModel->getTable().' WHERE added_by = '.$this->getUserId().') OR id IN (SELECT id FROM '.$pjUserModel->getTable().' WHERE added_by IN(SELECT id FROM '.$pjUserModel->getTable().' WHERE added_by = '.$this->getUserId().')))');	
                }
                ...
                if ($this->isBusAdmin()) {
                    $data_object->where('(t1.id = '.$this->getUserId().' OR t1.id IN (SELECT id FROM '.$pjUserModel->getTable().' WHERE added_by = '.$this->getUserId().') OR t1.id IN (SELECT id FROM '.$pjUserModel->getTable().' WHERE added_by IN(SELECT id FROM '.$pjUserModel->getTable().' WHERE added_by = '.$this->getUserId().')))');	
                }
