...
class pjApiBooking extends pjFrontMobile
{
    ...
    /* public function pjActionSaveBooking(){
     ...
     public function pjActionBooking() {
        ...
        $include_delay = (isset($_POST['include_delay']) && trim($_POST['include_delay'])) ? trim($_POST['include_delay']) : 'F';
        ...
        if ($pickup_id && $return_id && $booking_date && $bus_id && $seats && $first_name && $last_name && $phone) {
            ...
            if ($available_seats) {
                ...
                if(count($intersect) > 0) {
                    ...
                    $result = $this->requestAction(array('controller' => 'pjFrontEndMobile', 'action' => 'pjActionSaveBooking', 'params' => $params), array('return'));
                    if ($result['code'] == 200) {
                        /*$data = array(
                            'success' => 1,
                        ...
                        $return_data['travel_date'] = $booking_date_arr[0].'-'.$booking_date_arr[1].'-'.$booking_date_arr[2];
                        ...
                        if(isset($is_return) && ($is_return == 'T')){
                            ...
                            $return_date_arr = explode("/",  $return_date);
                            $return_date_formatted = $return_date_arr[0].'-'.$return_date_arr[1].'-'.$return_date_arr[2];
                            $return_data['return_date'] = $return_date_formatted;
            ...
            if(count($booking_ids_arr)){
                foreach($booking_ids_arr as $k => $id){
                    if($id > 0){
                        ...
                        $booking_details = $pjBookingModel->reset()->select('total,status,c_phone')->find($id)->getData ();
                        ...
                        if($status == 'confirmed'){
                            ...
                            $pjBookingModel->reset()->set('id', $id)->modify(
                                array(
                           ...
                           //send sms to the user
                            $sms_message = 'Your booking on '.PJ_SITE_NAME.' is confirmed';
                            $params = array(
                                    'text' => $sms_message,
                                    'type' => 'unicode',
                                    'key' => md5($this->option_arr['private_key'] . PJ_SALT)
                            );
                            $params['number'] = $booking_details['c_phone'];
    ...
    //New booking api for latest kind of booking(individual booking save for each seat person)

    public function pjActionBookingV2() {
        $param_data = file_get_contents("php://input");
        $post_data = json_decode($param_data);

        $pickup_id = (isset($post_data->departure_city_id) && trim($post_data->departure_city_id)) ? trim($post_data->departure_city_id) : '';
        $return_id = (isset($post_data->destination_city_id) && trim($post_data->destination_city_id)) ? trim($post_data->destination_city_id) : '';
        $booking_date = (isset($post_data->booking_date) && trim($post_data->booking_date)) ? trim($post_data->booking_date) : '';
        $bus_id = (isset($post_data->bus_id) && trim($post_data->bus_id)) ? trim($post_data->bus_id) : '';
        $is_return = (isset($post_data->is_return) && trim($post_data->is_return)) ? trim($post_data->is_return) : 'F';
        $return_date = (isset($post_data->return_date) && trim($post_data->return_date)) ? trim($post_data->return_date) : '';
        $return_bus_id = (isset($post_data->return_bus_id) && trim($post_data->return_bus_id)) ? trim($post_data->return_bus_id) : '';//for ret
        $is_user = (isset($post_data->is_user) && (trim($post_data->is_user) == 'T')) ? 'T' : 'F';
        $user_id = (isset($post_data->user_id) && trim($post_data->user_id)) ? trim($post_data->user_id) : 0;
        $total_price = (isset($post_data->total_price) && trim($post_data->total_price)) ? trim($post_data->total_price) : 0;
        $payment_status = (isset($post_data->payment_status) && trim($post_data->payment_status)) ? trim($post_data->payment_status) : '';
        $request_id = (isset($post_data->request_id) && trim($post_data->request_id)) ? trim($post_data->request_id) : '';
        $include_delay = (isset($post_data->include_delay) && trim($post_data->include_delay)) ? trim($post_data->include_delay) : 'F';
        $persons_data = isset($post_data->data) ? $post_data->data : array(); 
        $group_identifier = pjApiBooking::createGroupIdentifier();
        $payment_method = (isset($post_data->payment_method) && trim($post_data->payment_method)) ? trim($post_data->payment_method) : 'cash';
        $agent_discount = (isset($post_data->agent_discount) && trim($post_data->agent_discount)) ? trim($post_data->agent_discount) : 0;
        $discount_per_head = 0;
        if($agent_discount > 0){
            $total_num_of_people = count($persons_data);
            $discount_per_head = $agent_discount/$total_num_of_people;
        }
        $total_persons_count = count($persons_data);
        ...
        if ($pickup_id && $return_id && $booking_date && $bus_id) {
            ...
            $total_discount = 0;
            $seats_booked_resp = '';
            if(count($persons_data)){  //persons loop 
               
                foreach($persons_data as $keyVal => $person){

            $seats = (isset($person->seat_ids) && trim($person->seat_ids)) ? trim($person->seat_ids) : '';
            $seats = rtrim($seats, ',');
            $seats_booked = explode(',',$seats);
            $seats_count = count($seats_booked);
            //$selected_seats = implode('|',$seats_booked);
            ...
            //return seats
            if(isset($is_return) && ($is_return == 'T')){
               $return_seats = (isset($person->return_seat_ids) && trim($person->return_seat_ids)) ? trim($person->return_seat_ids) : '';
                $return_seats = rtrim($return_seats, ',');
                $return_seats_booked = explode(',',$return_seats);
                $return_seats_count = count($return_seats_booked);
                $r_arr = pjBusModel::factory()->find($return_bus_id)->getData();
            ...
            if ($ticket_id) {
                ...
                $form = array(
                    'step_checkout' => 1,
                    'c_title' => !empty($person->title) ? $person->title : '',
                    'c_fname' => $person->person_firstname,
                    'c_lname' => !empty($person->person_lastname) ? $person->person_lastname : '',
                    'c_phone_country' => $person->person_country_id,
                    'c_phone' => $person->person_phone,
                    'c_email' => !empty($person->person_email) ? $person->person_email : '',
                    'c_company' => '',
                    'c_notes' => '',
                    'c_address' => '',
                    'c_city' => '',
                    'c_state' => '',
                    'c_zip' => '',
                    'c_country' => $person->person_country_id,
            ...
            if ($available_seats) {
                ...
                if(count($intersect) > 0) {
                    ...
                    $result = $this->requestAction(array('controller' => 'pjFrontEndMobile', 'action' => 'pjActionSaveBookingV2', 'params' => $params), array('return'));
                    $return_data_data = array();
                    if ($result['code'] == 200) {
                        //reduce credit limit of agent if booking status is confirmed and booking done by agent
                        if(isset($is_agent_booking) && ($is_agent_booking == 1) && isset($result['additional_data']['total_price']) && ($result['additional_data']['status'] == 'confirmed')){
                            $new_credit = $user_credit[0]['credit'] - $result['additional_data']['total_price'];
                        ...
                        //$return_data[''][$keyVal] = array();
                        $return_data['booking_status'] = $result['additional_data']['status'];
                        $return_data['user_id'] = $result['additional_data']['user_id'];
                        ...
                        $return_data['arrival_time'] = $result['additional_data']['arrival_time'];
                        $return_data['payment_mode'] = $result['additional_data']['payment_mode'];
                        $return_data['group_identifier'] = $result['additional_data']['group_identifier'];

                        //$return_data['data'][$keyVal] = array();
                        
                        $return_data_data['booking_id'] = strval($result['booking_id']);
                        $return_data_data['unique_id'] = $result['additional_data']['unique_id'];
                        $return_data_data['title'] = $person->title;
                        $return_data_data['person_firstname'] = $result['additional_data']['person_firstname'];
                        $return_data_data['person_lastname'] = $result['additional_data']['person_lastname'];
                        $return_data_data['person_phone'] = $result['additional_data']['person_phone'];
                        $return_data_data['person_email'] = !empty($result['additional_data']['person_email']) ? $result['additional_data']['person_email'] : "";

                        //$return_data_data['total_price'] = $result['additional_data']['total_price'];
                        //$return_data_data['discount'] = $result['additional_data']['discount'];

                        $total_amount += $result['additional_data']['total_price'];
                        $total_discount += $result['additional_data']['discount'];
                        //$return_data['total_no_of_seatsbooked'] = $result['additional_data']['total_no_of_seatsbooked'];
                        $return_data_data['total_no_of_seatsbooked'] = $seats;
                        if(!empty($seats_booked_resp)){
                            $seats_booked_resp .= ','.$seats;
                        }else{
                            $seats_booked_resp .= $seats;
                        }
                        ...
                        //for return trip in case of round trip
                        if(isset($is_return) && ($is_return == 'T')){
                            $return_data_data['r_booking_id'] = strval($result['additional_data']['r_booking_id']);
                            $return_data_data['r_unique_id'] = $result['additional_data']['r_unique_id'];
                            ...
                            //$return_data_data['r_total_price'] = $result['additional_data']['r_total_price'];
                            //$return_data_data['r_discount'] = $result['additional_data']['r_discount'];
                            $total_amount += $result['additional_data']['r_total_price'];
                            $return_data_data['r_total_no_of_seatsbooked'] = $return_seats;
                            if(!empty($seats_booked_resp)){
                                $seats_booked_resp .= ','.$return_seats;
                            }else{
                                $seats_booked_resp .= $return_seats;
                            }
                        ...
                        //for return trip in case of round trip: ends
                        array_push($return_data['data'], $return_data_data);
                      
                       //booking success
                    } else {//else of status 200
                        $data = array(
                            'status' => 'false',
                            'message' => 'Some technical problem occured'
                        ); 
        ...
        $return_data['total_amount'] = strval($total_amount);
        $return_data['discount'] = number_format($total_discount, 2);
        $before_discount = $total_amount + $total_discount;
        $return_data['total_before_discount'] = number_format($before_discount, 2);
        $return_data['total_no_of_seatsbooked'] = $total_persons_count;
        
        $return_data['message'] = 'Booking successful';
        $data = array(
                'status' => 'true',
                'data' => $return_data
            ); 
    ...
    /*
     * Create random group_identifier number for group kind of bookings
     */
    public function createGroupIdentifier() {
        $length = 4;
        $chars = "123456789";
    ...
    /*
     * change booking status of all persons in a group
     */
    public function pjActionGroupStatus(){
        ...
        if(empty($group_identifier) || empty($agent_user_id) || empty($status) || empty($token)){
            $resp = array(
                        'status' => 'false',
        ...
        $token_validation_result = Token::validate($token, $this->jwt_secret);
        if($token_validation_result){
            //select booking ids with the group identifier
            $bookingResult = $pjBookingModel
                ->select('id')
                ->where('group_identifier', $group_identifier)
                ->findAll()
                ->getData();
            if($bookingResult){
                foreach($bookingResult as $booking_key => $booking_data){
