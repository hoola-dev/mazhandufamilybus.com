...
class pjApiNormalUser extends pjFront
{
    ...
    public function pjActionCreate(){
        ...
        $title = (isset($_POST['title'])) ? trim($_POST['title']) : '';//required for register
        $first_name = (isset($_POST['first_name'])) ? trim($_POST['first_name']) : '';//required for register
        $last_name = (isset($_POST['last_name'])) ? trim($_POST['last_name']) : '';//required for register
        $country = (isset($_POST['country'])) ? trim($_POST['country']) : '';//required for register
        ...
        if(empty($mobile)){
           ...
           pjAppController::jsonResponse($response); 
            ...
            if(!empty($pwd)){
                ...
                ->select('t1.id,t1.status,t1.email,t3.title,t3.first_name,t3.last_name,t3.country')
                ->join('pjRole', 't2.id=t1.role_id', 'left')
                ->join('pjUserExtraInfo', 't1.id=t3.user_id', 'left')
            ...
            //create user
            if(empty($title) || empty($first_name) || empty($last_name) || empty($email) || empty($country)){
                $missing_fields = '';
                if(empty($title)){
            ...
            try {
                ...
                $id_extra = pjUserExtraInfoModel::factory($userExtraData)->insert()->getInsertId();
            } catch (Exception $e) {
                $response = array(
                    'status' => 'false',
    ...
    /*
     * get user details of given user id
     */
    public function getUserDetails(){
        ...
        $pjUserModel = pjUserModel::factory();
        $userResult = $pjUserModel
        ->select('t1.email,t1.phone,t2.title,t2.first_name,t2.last_name,t2.country')
        ->join('pjUserExtraInfo', 't1.id=t2.user_id', 'left')
        ->where('t1.id', $user_id)
        ->where('t1.role_id', $this->normal_user_role_id)
        ->limit(1)
        ->findAll()
        ->getData();
        if((count($userResult) > 0) && isset($userResult[0]) && !empty($userResult[0])){
