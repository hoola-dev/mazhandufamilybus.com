...
class pjAdminBookings extends pjAdmin
{
	...
	public function pjActionGetBooking()
	{
		...
		if ($this->isXHR())
		{
            ...
            if ($this->isBusAdmin()) {
				//$pjBookingModel->where('(user_id = '.$this->getUserId().' OR user_id IN (SELECT id FROM '.$pjUserModel->getTable().' WHERE added_by = '.$this->getUserId().') OR user_id IN (SELECT id FROM '.$pjUserModel->getTable().' WHERE added_by IN(SELECT id FROM '.$pjUserModel->getTable().' WHERE added_by = '.$this->getUserId().')) OR user_id IN (SELECT id FROM '.$pjUserModel->getTable().' WHERE role_id = 2 AND agent_type = 3 AND added_by IN(SELECT id FROM '.$pjUserModel->getTable().' WHERE role_id = 1)))');	
			}
	...
	public function pjActionCreate()
	{
		...
		if ($this->isAdmin() || $this->isEditor() || $this->isBusAdmin())
		{
			if (isset($_POST['booking_create']))
			{
                ...
                $post_discount_on_ticket = $_POST['new_discount_on_ticket'];
                $booking_total = $_POST['pickup_total'];
                $booking_return_total = (isset($_POST['is_return']) && isset($_POST['return_bus_id'])) ? $_POST['return_total'] : 0;
                $total_booking = $booking_total + $booking_return_total - $post_discount_on_ticket;
				...
				if ($this->isEditor()) {
					...
					if ($booking_status == 'confirmed' && (empty($user_credit) || $user_credit[0]['credit'] < $total_booking)) {
						$err = 'CR01';
                ...
                $max_discount = ($bus_data['is_international_bus'] == 1) ? $this->option_arr['o_international_bus_discount'] : $this->option_arr['o_local_bus_discount'];
                
                $is_discount = false;
                if ($this->isAllowDiscount() && $post_discount_on_ticket > 0 && $max_discount >= $post_discount_on_ticket && $booking_total >= $post_discount_on_ticket) {
                    $is_discount = true;
                    $booking_total = $booking_total - $post_discount_on_ticket;
                    $_POST['pickup_total'] = $booking_total;
                    $data['discount_on_ticket'] = $post_discount_on_ticket;
                } else {
                    $post_discount_on_ticket = 0;
                }
				...
				$data['sub_total'] = $_POST['pickup_total']; /*$_POST['pickup_sub_total'];*/ unset($_POST['sub_total']);
				$data['tax'] = $_POST['pickup_tax']; unset($_POST['tax']);
				$data['total'] = $_POST['pickup_total']; unset($_POST['total']);
				$data['deposit'] = $_POST['pickup_total']; /*$_POST['pickup_deposit'];*/ unset($_POST['deposit']);
				...
				{
                    if ($is_discount == true && $post_discount_on_ticket > 0) {                    
                        $discount_item['booking_id'] = $id;
                        $discount_item['discount'] = $post_discount_on_ticket;                    
                        $discount_item['added_by'] = $this->getUserId();
                        $discount_item['added_on'] = date("Y-m-d H:i:s");
    
                        $result = pjDiscountModel::factory($discount_item)->insert()->getInsertId();
                    }
					...
					{
                        $is_discount = false;
                        if ($this->isAllowDiscount() && $post_discount_on_ticket > 0 && $max_discount >= $post_discount_on_ticket && $booking_return_total >= $post_discount_on_ticket) {
                            $is_discount = true;
                            $booking_total = $booking_return_total - $post_discount_on_ticket;                            
                            $_POST['return_total'] = $booking_total;                        
                            $data['discount_on_ticket'] = $post_discount_on_ticket;
                        } else {
                            $post_discount_on_ticket = 0;
                        }
						...
						$data['sub_total'] = $_POST['return_total']; /*$_POST['return_sub_total'];*/
						$data['tax'] = $_POST['return_tax'];
                        $data['total'] = $_POST['return_total'];
                        echo "<br>".$data['total'];
						$data['deposit'] = $_POST['return_total']; /*$_POST['return_deposit'];*/
						...
						{	
                            if ($is_discount == true && $post_discount_on_ticket > 0) {                    
                                $discount_item['booking_id'] = $return_booking_id;
                                $discount_item['discount'] = $post_discount_on_ticket;                    
                                $discount_item['added_by'] = $this->getUserId();
                                $discount_item['added_on'] = date("Y-m-d H:i:s");
                            ...
                            if ($this->isEditor()) {
                                ...
                                $new_credit = $user_credit[0]['credit'] - $booking_return_total;
			...
			{      
                ...
                if ($this->isEditor()/* || $this->isBusAdmin()*/) {  
                    if ($_POST['is_from_outside_booking'] == 1 || $_POST['is_from_search_outside_booking'] == 1) {
                        ...
                        if ($this->isBusAdmin()) {
                            //$sub_query = 'SELECT id FROM '.$pjUserModel->getTable().' WHERE added_by = '.$this->getUserId().' OR id IN (SELECT id FROM '.$pjUserModel->getTable().' WHERE added_by IN(SELECT id FROM '.$pjUserModel->getTable().' WHERE added_by = '.$this->getUserId().')) OR id IN (SELECT id FROM '.$pjUserModel->getTable().' WHERE role_id = 2 AND agent_type = 3 AND added_by IN(SELECT id FROM '.$pjUserModel->getTable().' WHERE role_id = 1))';                            
                        } 
                ...
                $is_bus_started = ($arr['booking_datetime'] > date('Y-m-d H:i:s')) ? false : true;

                if ($arr['status'] == 'cancelled' && $arr['status'] != $_POST['status']) {
                    $store['pickup_id'] = $pickup_id = $_POST['pickup_id'];
                    $store['return_id'] = $return_id = $_POST['return_id'];
                    $store['date'] = $booking_date = $_POST['booking_date'];
                    $bus_id = $_POST['bus_id'];
                    $seats_booked = explode('|',$_POST['selected_seats']);
                    ...
                    if(!empty($intersect)) {
                        pjUtil::redirect(PJ_INSTALL_URL. "index.php?controller=pjAdminBookings&action=pjActionUpdate&id=".$_POST['id']."&err=NAA");	
                        exit;
                    }
                }
                ...
                $max_discount = ($bus_arr['is_international_bus'] == 1) ? $this->option_arr['o_international_bus_discount'] : $this->option_arr['o_local_bus_discount'];

                $total_discount_arr = pjDiscountModel::factory()
					->select('SUM(t1.discount) AS total_discount')
					->where('booking_id',$_POST['id'])
                    ->findAll()->getData();

                $total_discount = (isset($total_discount_arr[0]) && $total_discount_arr[0]) ? $total_discount_arr[0]['total_discount'] : 0;
                    
                $remaining_discount = $max_discount - $total_discount;
                ...
                $is_discount = false;
                if ($this->isAllowDiscount() && $post_discount_on_ticket > 0 && $remaining_discount >= $post_discount_on_ticket && $booking_total >= $post_discount_on_ticket) {
                    $is_discount = true;
                    $booking_total = $booking_total - $post_discount_on_ticket;
                    $_POST['total'] = $_POST['sub_total'] = $_POST['deposit'] = $booking_total;
                    $data['discount_on_ticket'] = $total_discount + $post_discount_on_ticket;
                } else {
                    $post_discount_on_ticket = 0;
                }
                $booking_status = $_POST['status'];	
				...
				if ($user_arr && $user_arr['role_id'] == 2) {
					...
					if ($user_id == $this->getUserId() && $arr['status'] != $booking_status && $booking_status == 'confirmed' && (empty($user_credit) || $increase_credit < $booking_total)) {
                        $err = 'CRU01';
                    ...
                    $user_current_credit = (empty($user_credit)) ? 0 : $user_credit[0]['credit'];	
                    if ($arr['status'] != $booking_status && $booking_status == 'confirmed' && (empty($user_credit) || $user_current_credit < $booking_total)) {
                        $err = 'CRU01';
                        $ob = ($_POST['is_from_outside_booking'] == 1 || $_POST['is_from_search_outside_booking'] == 1) ? '&ob=1' : '';
                        pjUtil::redirect(PJ_INSTALL_URL. "index.php?controller=pjAdminBookings&action=pjActionUpdate&id=".$booking_edit_id."&err=$err$ob");	
                        exit;
                ...
                if ($arr['status'] != $_POST['status']) {
                    $data['status_updated_by'] = $this->getUserId();
                    $data['status_updated_on'] = date("Y-m-d H:i:s");
                    if ($this->isEditor() && $arr['user_id'] == 0) {
                        $data['user_id'] = $this->getUserId();
                    }
                }
                ...
                if ($is_discount == true) {                    
                    $discount_item['booking_id'] = $_POST['id'];
                    $discount_item['discount'] = $post_discount_on_ticket;                    
                    $discount_item['added_by'] = $this->getUserId();
                    $discount_item['added_on'] = date("Y-m-d H:i:s");
				...
				if ($user_arr && $user_arr['role_id'] == 2) {
					...
					if (!$is_bus_started && $booking_status == 'confirmed') {
						$result = pjCreditModel::factory()->whereIn('user_id', $user_id)->modifyAll(array(
							'credit' => $new_credit
						));	
					}
                } else if ($this->isEditor()) {                    
                    $new_credit = $user_current_credit - $booking_total;
                    
                    if (!$is_bus_started && $booking_status == 'confirmed') {
						$result = pjCreditModel::factory()->whereIn('user_id', $this->getUserId())->modifyAll(array(
							'credit' => $new_credit
						));	
					}
                }
                ...
                if ($arr['status'] != $_POST['status'] && $_POST['status'] == 'cancelled') {
                    $result = $this->requestAction(array('controller' => 'pjCombinedApi', 'action' => 'pjActionCancelInsurance', 'params' => array('uuid'=>$arr['uuid'])), array('return'));
                }
                ...
                if ($arr['status'] != $_POST['status']) {
                    ...
                    if (!$is_bus_started && $arr['payment_method'] == 'flutterwave' && $_POST['status'] == 'cancelled') {
                        $refund_status = $this->pjActionFlutterwaveRefund($arr['txn_id'],$arr['total']);
                    } 
                ...
                $total_discount = (isset($total_discount_arr[0]['total_discount']) && $total_discount_arr[0]['total_discount'] > 0) ? $total_discount_arr[0]['total_discount'] : 0;
                $this->set('total_discount',$total_discount);

                if ($max_discount <= $arr['total']) {
                    $remaining_discount = $max_discount - $total_discount;
                } else {
                    $remaining_discount = $arr['total'];
                }
                $this->set('remaining_discount',$remaining_discount);
                
                $discount_arr = pjDiscountModel::factory()
                    ->select("t1.*, t2.name")
                    ->join('pjUser', 't2.id = t1.added_by', 'left')
	...
	public function pjActionGetTickets()
	{
		...
		if ($this->isXHR())
		{
			...
			$locations = pjRouteCityModel::factory()
				...
				->select("t1.*, t2.departure_time, t2.arrival_time, t2.day_num" )
			...
			if(!empty($location_id_arr))
			{
                ...
                $max_discount = ($arr['is_international_bus'] == 1) ? $this->option_arr['o_international_bus_discount'] : $this->option_arr['o_local_bus_discount'];
	...
	public function pjActionResend()
	{
		...
		if ($this->isAdmin() || $this->isBusAdmin() || $this->isEditor())
		{
			if (isset($_POST['reminder']))
			{
                ...
                $lang_sms_message = pjMultiLangModel::factory()->reset()->select('t1.*')
                    ->where('t1.model','pjOption')
                    ->where('t1.locale', $this->getLocaleId())
                ...
                $this->set('lang_sms_message', $lang_sms_message[0]['content']);
    ...
    public function pjActionNotificationSend($option_arr, $booking_arr, $salt, $opt){
        ...
        $admin_emails = $this->getAdminBusAdminEmails();
        $admin_phones = $this->getAdminBusAdminPhones();
        ...
        //admin emails and phones
        if ($admin_emails && $option_arr['o_admin_email_confirmation'] == 1 && ($booking_arr['status'] == 'confirmed' || $booking_arr['status'] == 'pending'))
		{	
			$lang_message = $pjMultiLangModel->reset()->select('t1.*')
											 ->where('t1.model','pjOption')
											 ->where('t1.locale', $locale_id)
        ...
        if(!empty($admin_phones) && ($booking_arr['status'] == 'confirmed' || $booking_arr['status'] == 'pending'))
		{
			$lang_message = $pjMultiLangModel->reset()->select('t1.*')
												 ->where('t1.model','pjOption')
												 ->where('t1.locale', $locale_id)
        ...
        if ($option_arr['o_admin_email_cancel'] == 1 && $booking_arr['status'] == 'cancelled' && !empty($admin_emails))
		{	
			$lang_message = $pjMultiLangModel->reset()->select('t1.*')
											 ->where('t1.model','pjOption')
											 ->where('t1.locale', $locale_id)
        ...
        if(!empty($admin_phones) && $booking_arr['status'] == 'cancelled')
		{
			$lang_message = $pjMultiLangModel->reset()->select('t1.*')
												 ->where('t1.model','pjOption')
												 ->where('t1.locale', $locale_id)
