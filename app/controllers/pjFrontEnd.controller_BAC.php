...
class pjFrontEnd extends pjFront
{
		...
		if ($is_api || $this->isXHR ()) {
			...
			if ($id !== false && ( int ) $id > 0) {
				...
				if ($is_return == 'T') {
					...
					pjFrontEnd::pjActionConfirmSend ( $this->option_arr, $return_arr, PJ_SALT, 'pending');
				}
				...
				$additional_data['unique_id'] = $data['uuid'];
				$additional_data['name'] = $data['booking_route'];
				...
				$additional_data['departure_time'] = $arr['departure_time'];
				$additional_data['arrival_time'] = $arr['arrival_time'];
	...
	public function pjActionConfirmSend($option_arr, $booking_arr, $salt, $opt, $is_send_to_admin=true)
	{
		$Email = new pjEmail();
        ...
        if ($booking_arr['c_email'] && $opt == 'pending')
		{
            ...
            $email_subject = ($booking_arr['payment_method'] == 'cash') ? 'Your free seat reservation on order '.$booking_arr['uuid'] : $lang_message[0]['content'];
						   
			if (count($lang_message) === 1)
			{
                $message = str_replace($tokens['search'], $tokens['replace'], $lang_message[0]['content']);
                
                $_SESSION['cash_message'] = $_SESSION['cash_message'].'<br><br>'.$message;
        ...
        if(!empty($booking_arr['c_phone']) && $opt == 'pending')
		{
			...
			if (count($lang_message) === 1)
			{
                ...
                $_SESSION['cash_message'] = $_SESSION['cash_message'].'<div style="margin-bottom:30px;width:100%;border-bottom:1px solid #999999;">&nbsp;</div>'.$message;
