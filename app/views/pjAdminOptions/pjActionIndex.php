			...
			if ($count > 0)
			{
				...
				for ($i = 0; $i < $count; $i++)
				{
					...
					<tr class="pj-table-row-odd<?php echo $rowClass; ?>" style="<?php echo $rowStyle; ?>">
						<td>
                        <?php if ($tpl['arr'][$i]['key'] == 'o_agent_email_confirm_message') { ?>
                            Email to Agents to confirm their Email-ID
                        <?php } else if ($tpl['arr'][$i]['key'] == 'o_agent_credit_message') { ?>
                            Message to Agents regarding the credit update
                        <?php } else { ?>
