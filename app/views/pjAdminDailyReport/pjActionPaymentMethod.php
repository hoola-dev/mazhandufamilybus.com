...
if (isset($tpl['status']))
{
    ...
    pjUtil::printNotice('Payment Method Report of '.$tpl['name'].' : '.date("F j, Y",strtotime($tpl['start_date'])).' - '.date("F j, Y",strtotime($tpl['end_date'])),'');
    $week_start = isset($tpl['option_arr']['o_week_start']) && in_array((int) $tpl['option_arr']['o_week_start'], range(0,6)) ? (int) $tpl['option_arr']['o_week_start'] : 0;
	$jqDateFormat = pjUtil::jqDateFormat($tpl['option_arr']['o_date_format']);
?>
    <div style="clear:both;height:auto;overflow:hidden;">
        <form id="frm_daily_report" action="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminDailyReport&amp;action=pjActionPaymentMethod" method="post" id="frmUpdateTime" class="pj-form form">
            <div style="clear:both;overflow:hidden;">Payment Method</div>
            <div style="clear:both;overflow:hidden;">
                <select name="payment_method" id="payment_method">
                    <?php if ($tpl['payment_methods']) { ?>
                        <?php foreach ($tpl['payment_methods'] as $k => $v) { ?>
                            <option value="<?php echo $k; ?>" <?php if ($k == $tpl['payment_method']) { ?> selected <?php } ?>><?php echo $v; ?></option>
                        <?php } ?>
                    <?php } ?>
                </select>
            </div>
