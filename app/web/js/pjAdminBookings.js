...
(function ($, undefined) {
	$(function () {
        ...
        function onBeforeShow (obj) {
			if (pjGrid.currentRoleId == 2 || pjGrid.currentRoleId == 10) {
				return false;
			}
			return true;
		}
		...
		if ($("#grid").length > 0 && datagrid) {
			var $grid = $("#grid").datagrid({
				buttons: [{type: "edit", url: "index.php?controller=pjAdminBookings&action=pjActionUpdate&id={:id}", beforeShow: onBeforeEditShow},
				          {type: "delete", url: "index.php?controller=pjAdminBookings&action=pjActionDeleteBooking&id={:id}", beforeShow: onBeforeShow}
						  ],
				columns: [
		...
		$(document).on("focusin", ".datepick", function (e) {
			...
			if($('#bus_id'). val() != '')
			{
				...
				$.post("index.php?controller=pjAdminBookings&action=pjActionGetTickets", frm.serialize()).done(function (data) {
                    ...
                    remaining_discount = parseInt(data.max_discount);
                    $('#discount_p').html(data.discount_html);
...
function check_discount(discount) {
    if (remaining_discount == 0) {
        alert('Please note that you could not give more discount');
        jQuery_1_8_2('#new_discount_on_ticket').val(remaining_discount);
    } else if (discount > remaining_discount) {
        alert('Please note that you could give only upto '+remaining_discount+' as discount');
        jQuery_1_8_2('#new_discount_on_ticket').val(remaining_discount);
    }
}