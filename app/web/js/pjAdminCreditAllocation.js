...
(function ($, undefined) {
	$(function () {
		...
		if ($("#grid").length > 0 && datagrid) {
			...
			var $grid = $("#grid").datagrid({
				...
				columns: [
                          ...
                          {text: myLabel.cm_credit_added, type: "text", sortable: true, editable: false},
				...
				dataUrl: "index.php?controller=pjAdminAgents&action=pjActionGetCreditAllocation&id",
				dataType: "json",
				fields: ['name','ca_credit_added', 'ca_commission_percent', 'ca_commission', 'ca_total_commission', 'ca_added_on'],
				paginator: {					
					gotoPage: true,
					paginate: true,
					total: true,
		...
		$(document).on("click", ".btn-all", function (e) {
			...
			$grid.datagrid("load", "index.php?controller=pjAdminAgents&action=pjActionGetCreditAllocation", "name", "ASC", content.page, content.rowCount);
			return false;
			...
			$.post("index.php?controller=pjAdminAgents&action=pjActionSetActive", {
				...
				$grid.datagrid("load", "index.php?controller=pjAdminAgents&action=pjActionGetCreditAllocation");
			});
			return false;
			...
			$grid.datagrid("option", "cache", cache);
			$grid.datagrid("load", "index.php?controller=pjAdminAgents&action=pjActionGetCreditAllocation", "id", "ASC", content.page, content.rowCount);
			return false;
		});
	});
