...
(function ($, undefined) {
	$(function () {
			...
			$frmCreateUser.validate({
				rules: {
					"email": {
						required: true,
						email: true,
						remote: "index.php?controller=pjAdminAppUsers&action=pjActionCheckEmail"
					}
				},
				messages: {
					"email": {
			...
			$frmUpdateUser.validate({
				rules: {
					"email": {
						required: true,
						email: true,
						remote: "index.php?controller=pjAdminAppUsers&action=pjActionCheckEmail&id=" + $frmUpdateUser.find("input[name='id']").val()
					}
				},
				messages: {
					"email": {
        ...
        function onBeforeShow2 (obj) {
			if (parseInt(obj.agent_type, 10) != 2) {
				return false;
			}
			return true;
		}
		...
		if ($("#grid").length > 0 && datagrid) {
			...
			var $grid = $("#grid").datagrid({
				buttons: [{type: "edit", url: "index.php?controller=pjAdminAppUsers&action=pjActionUpdate&id={:id}"},						  
                          {type: "delete", url: "index.php?controller=pjAdminAppUsers&action=pjActionDeleteUser&id={:id}", beforeShow: onBeforeShow},
                          {type: "search", url: "index.php?controller=pjAdminAppUsers&action=pjActionSubAgents&id={:id}", beforeShow: onBeforeShow2}
				          ],
				columns: [{text: myLabel.name, type: "text", sortable: true, editable: true},
                          ...
                          {text: myLabel.at_type, type: "text", sortable: true, editable: true},
				          {text: myLabel.status, type: "select", sortable: true, editable: true, options: [
						{label: myLabel.active, value: "T"}, 
						{label: myLabel.inactive, value: "F"}
						], applyClass: "pj-status"}],
				dataUrl: "index.php?controller=pjAdminAppUsers&action=pjActionGetUser",
				dataType: "json",
				fields: ['name', 'email', 'created','role', 'at_type', 'status'],
				paginator: {
					actions: [
					   {text: myLabel.delete_selected, url: "index.php?controller=pjAdminAppUsers&action=pjActionDeleteUserBulk", render: true, confirmation: myLabel.delete_confirmation},
					   {text: myLabel.revert_status, url: "index.php?controller=pjAdminAppUsers&action=pjActionStatusUser", render: true},
					   {text: myLabel.exported, url: "index.php?controller=pjAdminAppUsers&action=pjActionExportUser", ajax: false}
					],
					gotoPage: true,
					paginate: true,
					total: true,
				...
				saveUrl: "index.php?controller=pjAdminAppUsers&action=pjActionSaveUser&id={:id}",
				select: {
					field: "id",
					name: "record[]"
				}
		...
		$(document).on("click", ".btn-all", function (e) {
			...
			$grid.datagrid("load", "index.php?controller=pjAdminAppUsers&action=pjActionGetUser", "name", "ASC", content.page, content.rowCount);
			return false;
			...
			$.post("index.php?controller=pjAdminAppUsers&action=pjActionSetActive", {
				id: $(this).closest("tr").data("object")['id']
			}).done(function (data) {
				$grid.datagrid("load", "index.php?controller=pjAdminAppUsers&action=pjActionGetUser");
			});
			return false;
			...
			$grid.datagrid("option", "cache", cache);
			$grid.datagrid("load", "index.php?controller=pjAdminAppUsers&action=pjActionGetUser", "id", "ASC", content.page, content.rowCount);
			return false;
		});
	});
...
function save_credit(){
	...
	jQuery_1_8_2("#frmUpdateCredit").fadeTo('slow',0.2,function(){
		jQuery_1_8_2.post(
			base_url+'index.php?controller=pjAdminAppUsers&action=pjActionCreditUpdate&rand='+Math.random(),
			post_data,
			function(data){
				if (data['success']) {
					window.location.href = base_url+'index.php?controller=pjAdminAppUsers&action=pjActionIndex'; 
				} else {
					alert(data['message']);
				}
